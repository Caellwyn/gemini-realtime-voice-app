class PCMProcessor extends AudioWorkletProcessor {
    ...
    process(inputs, outputs, parameters) {
        ...
        if (this.buffer.length >= channelData.length) {
            ...
            // In test mode, we want to hear the audio.
            // In normal mode, we don't, so we would return false or not set the output.
            // For simplicity in test mode, we always process.
            return true;
        }

        // Mute output if buffer is empty
        channelData.fill(0);
        return true;
    }
}
